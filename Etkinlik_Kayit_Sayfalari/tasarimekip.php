...
<html>
	<head>
		...
		<title>StartupMEF Tasarım Ekibi Kayıt Form</title>
		<meta charset="utf-8" />        
...
<body>
            <!-- multistep form -->
            <form id="msform" method="post" action="tasarimekip.php">
            <!-- progressbar -->
            <ul id="progressbar">
            ...
            <!-- fieldsets -->
            <fieldset>
                <h2 class="fs-title">Tasarım Ekip Üyesi Kayıt</h2>
            ...
            <fieldset>
                ...
                <SELECT type="text" name="deneyim" required>
                    <OPTION value="" disabled selected>Tasarım düzenleme ve yönetimi hakkında önceden staj veya proje deneyimin oldu mu?</option>
                    <OPTION> Evet
                    <OPTION> Hayır
                </SELECT>
                ...
                <textarea type="text" name="sosyalmedya" placeholder="Seni etkileyen sosyal medya sayfaları nelerdir? (Tasarımsal)" required></textarea>
                <textarea type="text" name="dikkat" placeholder="Hangi tasarım düzenleme uygulamalarını kullanıyorsun?" required></textarea>
...
if(isset($_POST["name"], $_POST["surname"], $_POST["class"], $_POST["email"], $_POST["telefon"],$_POST["deneyimbah"], $_POST["squad"], $_POST["deneyim"], $_POST["kulup"], $_POST["beklenti"], $_POST["dikkat"], $_POST["deneyimyon"], $_POST["sosyalmedya"], $_POST["eklemek"])){
    ...
    $ekle = "INSERT INTO Tasarimekip (Adin, Soyadin, Sinifin, Mail_Adresin, Telefon_Numaran, Squad, Daha_once_deneyim, deneyimbah, Beklenti, Hangi_sosyalmedya, Dikkat_edilecek_sey, deneyimyon , Hangi_kulup, eklemek) VALUES ('".addslashes($ad)."','".addslashes($soyad)."','".addslashes($sınıf)."','".addslashes($email)."','".addslashes($tel)."','".addslashes($squad)."','".addslashes($deneyim)."', '".addslashes($deneyimbah)."','".addslashes($beklenti)."','".addslashes($sosyalmedya)."','".addslashes($dikkat)."','".addslashes($deneyimyon)."','".addslashes($kulup)."','".addslashes($eklemek)."')";
