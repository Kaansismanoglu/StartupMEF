...
<html>
	<head>
		...
		<title>StartupMEF İçerik Ekibi Kayıt Form</title>
		<meta charset="utf-8" />        
...
<body>
            <!-- multistep form -->
            <form id="msform" method="post" action="icerikekibi.php">
            <!-- progressbar -->
            <ul id="progressbar">
            ...
            <!-- fieldsets -->
            <fieldset>
                <h2 class="fs-title">İçerik Ekip Üyesi Kayıt</h2>
            ...
            <fieldset>
                ...
                <SELECT type="text" name="hangiekip" required>
                    <OPTION value="" disabled selected>İçerik ekibinin hangi alanına başvurmak istiyorsun?</option>
                    <OPTION> Blog(Medium) / Organizasyon
                    <OPTION> Podcast / Podcast Düzenleme
                    <OPTION> Vlog
                </SELECT>

                <SELECT type="text" name="deneyim" required>
                    <OPTION value="" disabled selected>Podcast veya blog alanlarıyla ilgili önceden staj veya proje deneyimin oldu mu?</option>
                    <OPTION> Evet
                    <OPTION> Hayır
                </SELECT>
                ...
                <textarea type="text" name="sosyalmedya" placeholder="Seni etkileyen blogger / podcastler / YouTube kanalları nelerdir?" required></textarea>
            ...
            <fieldset>
                ...
                <input type="button" name="previous" class="previous action-button" value="Önceki" />
                <input type="submit" class="primary" value="Gönder" />
            </fieldset>
            </form>
...
if(isset($_POST["name"], $_POST["surname"], $_POST["class"], $_POST["email"], $_POST["telefon"],$_POST["deneyimbah"],$_POST["hangiekip"], $_POST["squad"], $_POST["deneyim"], $_POST["kulup"], $_POST["beklenti"], $_POST["deneyimyon"], $_POST["sosyalmedya"], $_POST["eklemek"])){
    ...
    $hangiekip = $_POST["hangiekip"];
    $deneyimbah = $_POST["deneyimbah"];
    $sosyalmedya = $_POST["sosyalmedya"];
    $eklemek = $_POST["eklemek"];


    $ekle = "INSERT INTO icerikekibi (Adin, Soyadin, Sinifin, Mail_Adresin, Telefon_Numaran, Squad, Hangiekip ,Daha_once_deneyim, deneyimbah, Beklenti, Hangi_sosyalmedya, deneyimyon , Hangi_kulup, eklemek) VALUES ('".addslashes($ad)."','".addslashes($soyad)."','".addslashes($sınıf)."','".addslashes($email)."','".addslashes($tel)."','".addslashes($squad)."','".addslashes($hangiekip)."','".addslashes($deneyim)."','".addslashes($deneyimbah)."','".addslashes($beklenti)."','".addslashes($sosyalmedya)."','".addslashes($deneyimyon)."','".addslashes($kulup)."','".addslashes($eklemek)."')";
