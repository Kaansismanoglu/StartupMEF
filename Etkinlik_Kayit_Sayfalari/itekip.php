...
<html>
	<head>
		...
		<title>StartupMEF IT Ekibi Kayıt Form</title>
		<meta charset="utf-8" />        
...
<body>
            <!-- multistep form -->
            <form id="msform" method="post" action="itekip.php">
            <!-- progressbar -->
            <ul id="progressbar">
            ...
            <!-- fieldsets -->
            <fieldset>
                <h2 class="fs-title">IT Ekip Üyesi Kayıt</h2>
            ...
            <fieldset>
                ...
                <SELECT type="text" name="deneyim" rows="2" required>
                    <OPTION value="" disabled selected>Web sitesi, discord kanalı ya da yazılım ile ilgili herhangi bir konu hakkında daha önceden staj veya proje deneyimin oldu mu?</option>
                    <OPTION> Evet
                    <OPTION> Hayır
                </SELECT>
                ...
                <textarea type="text" name="websitesi" placeholder="Tasarım açısından beğendiğin 3 adet websitesinden bahsedebilir misin?" required></textarea>
                <textarea type="text" name="yorum" placeholder="StartupMEF web sitesini yorumlayacak olsan neler eklemek isterdin?" required></textarea>
            ...
            <fieldset>
                ...
                <textarea type="text" name="yazilimdil" placeholder="Bildiğin yazılım dilleri neler? Bu dillerde ne kadardır deneyimin var?" required></textarea>
                <textarea type="text" name="deneyimyon" placeholder="Kulüp üyeliği / yöneticiliği deneyimin oldu mu?" required></textarea>
...
if(isset($_POST["name"], $_POST["surname"], $_POST["class"], $_POST["email"], $_POST["telefon"],$_POST["deneyimbah"], $_POST["squad"], $_POST["deneyim"], $_POST["kulup"], $_POST["beklenti"],  $_POST["yazilimdil"], $_POST["deneyimyon"], $_POST["websitesi"],  $_POST["yorum"], $_POST["eklemek"])){
    ...
    $email = $_POST["email"];
    $tel = $_POST["telefon"];
    $deneyimyon = $_POST["deneyimyon"];
    $squad = $_POST["squad"];
    $deneyim = $_POST["deneyim"];
    $kulup = $_POST["kulup"];
    $yorum = $_POST["yorum"];
    $beklenti = $_POST["beklenti"];
    $yazilimdil = $_POST["yazilimdil"];
    $deneyimbah = $_POST["deneyimbah"];
    $websitesi = $_POST["websitesi"];
    $eklemek = $_POST["eklemek"];


    $ekle = "INSERT INTO itekip (Adin, Soyadin, Sinifin, Mail_Adresin, Telefon_Numaran, Squad, Daha_once_deneyim, deneyimbah, Beklenti, Hangi_websitesi, Websitesi_yorum, Yazilim_Dil ,deneyimyon , Hangi_kulup, eklemek) VALUES ('".addslashes($ad)."','".addslashes($soyad)."','".addslashes($sınıf)."','".addslashes($email)."','".addslashes($tel)."','".addslashes($squad)."','".addslashes($deneyim)."','".addslashes($deneyimbah)."','".addslashes($beklenti)."','".addslashes($websitesi)."','".addslashes($yorum)."','".addslashes($yazilimdil)."','".addslashes($deneyimyon)."','".addslashes($kulup)."','".addslashes($eklemek)."')";
